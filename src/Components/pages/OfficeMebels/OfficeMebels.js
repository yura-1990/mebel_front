...
function OfficeMebels({ officeMebels, officemebelsByID, curLang }) {
  ...
  return (
    ...
    <div className="container-fluid">
      <div className="row category-top mebel-list justify-content-center">
        <div className="col-12">
          {officeMebels && <div className="card shadow">
            ...
            officeMebels?.map((el, i)=>{
              return (
                <div key={i} className="mt-4 col-xl-3 col-lg-4 col-sm-6 co-12">
                  <Link to={`/mebel/officemebel/${el.id}`} onClick={()=>officemebelsByID(el.id)} className='bg-white text-custom rounded-3 d-flex h-100 p-3 overflow-hidden shadow'>
                    <div className="d-flex flex-column justify-content-between">
                      ...
                      <p className="text-center  m-0 mt-2">{ el?.name_ru?.split('/n')?.map(el=><p className="mb-0">{el}</p>)  ?? el?.name_uz?.split('/n')?.map(el=><p className="mb-0">{el}</p>)  }</p>
                    </div>
                  </Link>
                </div>
              )
    ...
    <div className='footer-distributed mt-5'>
...
})=>({officeMebels}), { officemebelsByID })(OfficeMebels)
