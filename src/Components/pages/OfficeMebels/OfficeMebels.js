            ...
            officeMebels?.map((el, i)=>{
              return (
                <div key={i} className="mt-4 col-xl-3 col-lg-4 col-sm-6 co-12">
                  <Link to={`/mebel/officemebel/${el.id}`} onClick={()=>officemebelsByID(el.id)} className='bg-white rounded-3 d-flex h-100 p-3 overflow-hidden shadow'>
                    <div className="d-flex flex-column justify-content-between">
                      ...
                      <p className="text-center m-0 mt-2">{el.name_uz ?? el.name_ru}</p>
                    </div>
                  </Link>
                </div>
              )
