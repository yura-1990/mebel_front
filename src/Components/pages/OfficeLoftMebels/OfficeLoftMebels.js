            ...
            loftOfficeMebels?.map((el, i)=>{
              return (
                <div key={i} className="mt-4 col-xl-3 col-lg-4 col-sm-6 co-12">
                  <Link to={`/mebel/loftmebel/${el.id}`}  onClick={()=>loftOfficeMebelByID(el.id)} className='bg-white text-custom rounded-3 d-flex h-100 p-3 overflow-hidden shadow'>
                    <div className="d-flex flex-column justify-content-between">
                      ...
                      <p className="text-center  m-0 mt-2">{ el?.name_ru?.split('/n')?.map((el,i)=><p key={i} className="mb-0">{el}</p>)  ?? el?.name_uz?.split('/n')?.map((el,i)=><p key={i} className="mb-0">{el}</p>) }</p>
                    </div>
                  </Link>
                </div>
              )
