...
import homBanner from '../../../img/banner-home.jpg'
import scrollDown from '../../../img/scroll-down.gif'
...
export default function Home({allMebelData,officeMebels,loftOfficeMebels,homeMebels,kitchenMebels,allSoftMebels, curLang}) {
  ...
  const [absoluteSize, setAbsoluteSize] = useState(null)
  const [scrollDowns, setScrollDowns] = useState(null)
  ...
  function handleImageLoad() {
    const image = document.querySelector('.image-size')
    const scrollDownElement = document.querySelector('.scroll-down')
    const percentage = (scrollDownElement.clientHeight / 100)
    const getPercentageOfScrollDown = Math.floor((windowHeight / 100) * (fixedHeight / 100)) * percentage
    console.log(getPercentageOfScrollDown);

    setImageHeight(image.clientHeight)
    setScrollDowns(getPercentageOfScrollDown)

    if (image.clientHeight > windowHeight) {
      setAbsoluteSize(windowHeight)
    }else {
      setAbsoluteSize(image.clientHeight)
    }
  };

  return (
    <div>
      <div className='home-banner position-relative d-flex justify-content-center align-items-center' style={{ height: absoluteSize + 'px', objectPosition: 'top', objectFit: 'cover'}}>
        <img src={homBanner} alt="home banner" className='img-fluid image-size' />
        <img src={scrollDown} alt="scroll down" className="scroll-down position-absolute" style={{ height: scrollDowns + 'px' }} />
      </div>
